...
Веб-интерфейс реестра. Открывается в браузере:

```
https://registry.example.com/ui
```

Возможности:
- Форма входа (те же пользователи что и для `docker login`)
- Статистика: количество репозиториев и pulls
- Список репозиториев с тегами, digest и размером образа
- Поиск репозиториев
- Удаление тегов

При `RGSTR_AUTH_ENABLED=false` форма входа не требует заполнения.
